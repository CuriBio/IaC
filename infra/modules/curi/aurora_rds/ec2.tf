...
resource "null_resource" "ssh_ec2_connection" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "mysql -u ${format(var.db_username)} -p${format(var.db_password)} -h ${module.rds.rds_cluster_instance_endpoints[0]} -P ${module.rds.rds_cluster_port} -e 'CREATE DATABASE test_db;'",
    ]
  }
}
