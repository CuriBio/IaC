...
def test_sdk_upload__returns_error_code_if_file_name_not_given():
    response = sdk_upload.handler({"body": json.dumps({})}, None)
    assert response == {
        "statusCode": 400,
...
def test_sdk_upload__calls_generate_presigned_post_with_correct_values(mocker, mocked_boto3_client):
    ...
    mocker.patch.object(sdk_upload, "S3_BUCKET", expected_bucket_name)
    spied_uuid4 = mocker.spy(sdk_upload.uuid, "uuid4")

    test_file_name = "test_file"
    sdk_upload.handler({"body": json.dumps({"file_name": test_file_name})}, None)

    expected_upload_id = str(spied_uuid4.spy_return)
    expected_fields = {"x-amz-meta-upload-id": expected_upload_id}
    expected_conditions = [{"x-amz-meta-upload-id": expected_upload_id}]
...
def test_sdk_upload__handles_info_logging(mocker, mocked_boto3_client):
    ...
    test_file_name = "test_file"
    test_event = {"body": json.dumps({"file_name": test_file_name})}
...
def test_sdk_upload__returns_correct_response_for_a_given_file_name(mocker, mocked_boto3_client):
    ...
    test_file_name = "test_file"
    response = sdk_upload.handler({"body": json.dumps({"file_name": test_file_name})}, None)
    assert response == {
        "statusCode": 200,
        "headers": {"Content-Type": "application/json"},
...
def test_sdk_upload__logs_exception_when_generate_presigned_post_raises_ClientError(
    ...
    with pytest.raises(ClientError):
        sdk_upload.handler({"body": json.dumps({"file_name": ""})}, None)
