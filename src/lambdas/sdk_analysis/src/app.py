...
if __name__ == '__main__':
    ...
    while True:
        try:
            sqs_response = sqs_client.receive_message(QueueUrl=SQS_URL, MaxNumberOfMessages=1, WaitTimeSeconds=10)
            ...
            for message in sqs_response.get('Messages', []):
                ...
                if message_body:
                    for record in message_body.get('Records', []):
                        if record.get('eventSource') == 'aws:s3' and record.get('eventName') == 'ObjectCreated:Post':
